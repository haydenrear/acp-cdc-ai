...
package modelcontextprotocol.client.transport;
 ...
 */
public class DelegatingHttpClientStreamableHttpTransport implements McpClientTransport {

	private static final Logger logger = LoggerFactory.getLogger(DelegatingHttpClientStreamableHttpTransport.class);
	...
	private DelegatingHttpClientStreamableHttpTransport(McpJsonMapper jsonMapper, HttpClient httpClient,
	...
	public Mono<Void> connect(Function<Mono<McpSchema.JSONRPCMessage>, Mono<McpSchema.JSONRPCMessage>> handler,
		...
		return Mono.deferContextual(ctx -> {
			...
			if (this.openConnectionOnStartup) {
				...
				return this.reconnect(null, ccBearer).doOnError(t -> {
	...
	private Mono<Disposable> reconnect(McpTransportStream<Disposable> stream, String bearer) {
	...
	public Mono<Void> sendMessage(McpSchema.JSONRPCMessage sentMessage, String bearer) {
		return Mono.create(deliveredSink -> {
			...
			Disposable connection = Mono.deferContextual(ctx -> {
				...
				if (transportSession.markInitialized(
					...
					reconnect(null, bearer).contextWrite(deliveredSink.contextView()).subscribe();
				}
							...
							.flatMap(sseEvent -> {
								try {
									...
									McpTransportStream<Disposable> sessionStream = new DefaultMcpTransportStream<>(
											this.resumableStreams, s -> this.reconnect(s, bearer));
	...
	/**
	 * Builder for {@link DelegatingHttpClientStreamableHttpTransport}.
	 */
	public static class Builder {
		...
		/**
		 * Construct a fresh instance of {@link DelegatingHttpClientStreamableHttpTransport} using
		 * the current builder configuration.
		 * @return a new instance of {@link DelegatingHttpClientStreamableHttpTransport}
		 */
		public DelegatingHttpClientStreamableHttpTransport build() {
			HttpClient httpClient = this.clientBuilder.connectTimeout(this.connectTimeout).build();
			var t = new DelegatingHttpClientStreamableHttpTransport(jsonMapper == null ? McpJsonMapper.getDefault() : jsonMapper,
			...
			t.setClientBuilder(clientBuilder);
			return t;
		}

	}

	public DelegatingHttpClientStreamableHttpTransport.Builder toBuilder() {
		return DelegatingHttpClientStreamableHttpTransport.builder(baseUri.toString())
				.endpoint(endpoint)
